Shader "GeometryObjSamples/S_HairPrime"
{
	Properties
	{
		...
		_SpecularScale ("Specular Scale(高光强度)", Range(0, 2)) = 1
	}
	SubShader
	{
		...
		Pass
		{
			...
			float HairSpecular(float3 t, float3 n, float3 l, float3 v, float2 uv)
			{
				...
				float3 speuclar = _SpecularColor_1 * StrandSpecular(t1, v, l, 20);
				return speuclar;
			}
			...
			{
				//diffuse color
				float3 diff = tex2D(_MainTex, i.uv).rgb * _MainColor;
				//specular
				...
				float4 col = float4(_LightColor0 * (spec + diff), 1);
				return col;
			}
