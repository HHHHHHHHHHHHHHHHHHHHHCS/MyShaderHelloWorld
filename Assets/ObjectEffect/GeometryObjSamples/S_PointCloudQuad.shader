Shader "GeometryObjSamples/S_PointCloudQuad"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			{
				g2f vertex[4];
				float4 screenParam = float4(1.0, 1.0 * _ScreenParams.x / _ScreenParams.y, 0, 0) * size;
				float4 center = UnityObjectToClipPos(pos);
				vertex[0] = (g2f)0;
				vertex[0].pos = center + float4(-0.5, -0.5, 0, 0) * screenParam;
				vertex[1] = (g2f)0;
				vertex[1].pos = center + float4(-0.5, 0.5, 0, 0) * screenParam;
				vertex[2] = (g2f)0;
				vertex[2].pos = center + float4(0.5, -0.5, 0, 0) * screenParam;
				vertex[3] = (g2f)0;
				vertex[3].pos = center + float4(0.5, 0.5, 0, 0) * screenParam;
				stream.Append(vertex[0]);
				stream.Append(vertex[2]);
				stream.Append(vertex[1]);
				stream.RestartStrip();
				stream.Append(vertex[1]);
				stream.Append(vertex[2]);
				stream.Append(vertex[3]);
				stream.RestartStrip();
			}
