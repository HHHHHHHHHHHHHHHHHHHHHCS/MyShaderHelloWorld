Shader "HCS/S_LensFlare_TonemappingColorGrading"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			#include "S_LensFlare_TonemappingColorGrading.cginc"
			...
			// CG的fmod()与glsl的mod()不同，如果是负值，我们将使用自己的
			...
			half4 frag_lut_gen(v2f_img i): SV_TARGET
			{
				...
				//Color Mixer
				final_lut = half3(dot(final_lut, _ChannelMixerRed), dot(final_lut, _ChannelMixerGreen), dot(final_lut, _ChannelMixerBlue));
