...
#define CANDLECOL vec3(.9333,1.,0.)
...
{
	float h=clamp(.5+.5*(b-a)/k,0.,1.);
	return vec2(mix(a,b,h)+k*h*(1.-h),h);
}
...
de map(vec3 p){
	...
	float os=sdCappedCylinder(p+vec3(0.,3.,0.),vec2(2.75,2.))-.1;
	float is=sdCappedCylinder(p-vec3(0.,3.,0.),vec2(2.45,2.))-.3;
	
	p.y+=(sin(p.x*10.)*sin(p.y*12.))*.01;
	float outside=sdCappedCylinder(p+vec3(0.,2.,0.),vec2(.55,2.))-.1;
	float inside=sdCappedCylinder(p-vec3(0.,2.,0.),vec2(.35,2.))-.3;
	
	vec2 candle0=smax(os,-is,.1);
	if(candle0.x<.05)
	{
		o.m=2.;
	}
	vec2 candle1=smax(outside,-inside,.1);
	
	vec3 q=p+vec3(0.,.15,0.);
	q=opCheapBend(q+vec3(0.,.2,0.)*0.,1.);
	
	float angle=atan(q.x,q.z);
	
	q.xz*=1.-abs(sin(angle*twopi+q.y*40.))*.02*S(.1,.2,q.y);
	q.xz*=1.-S(.2,.0,q.y)*.2;
	float wick=sdCappedCylinder(q+vec3(0.,.1,0.),vec2(.01,.7))-.05;
	
	vec2 d=smin(candle0.x,candle1.x,.1);
	d=smin(d.x,wick,.2);
...
de fmap(vec3 p,float n){
	...
	vec3 q=opCheapBend(p+vec3(0.,.2,0.),1.);
	
	float wick=sdCappedCylinder(q+vec3(0.,.1,0.),vec2(.01,.7))-.01;
	float d=wick;
	float flame=wick;
...
vec3 calcNormal(de o)
{
	...
	vec3 nor=vec3(map(o.pos+eps.xyy).d-map(o.pos-eps.xyy).d,
...
{
	vec3 eps=vec3(.001,0.,0.);
	vec3 nor=vec3(fmap(p+eps.xyy,n).d-fmap(p-eps.xyy,n).d,
...
vec4 render(vec2 uv,ray camRay,float n){
	...
	if(o.m>0.){
		...
		vec3 candleCol=vec3(0);
		if(o.m==2.)
		{
			candleCol=mix(vec3(1.),vec3(1.),inside);
		}
		else
		{
			candleCol=mix(CANDLECOL,vec3(1.),inside/2.);
		}
...
vec4 RenderCake(vec2 uv)
{
	float t=iTime;
	uv=(2.*uv-(iResolution.xy))/iResolution.y;// -1 <> 1
...
vec3 explosion(vec2 uv,vec2 p,float seed,float t){
	...
	for(float i=0.;i<NUM_PARTICLES;i++){
		...
		sparkle=pow(sparkle,pow(en.x,3.)*50.)*mix(.01,.01,en.y*n.y);
...
vec4 RenderFirework(in vec2 fragCoord)
{
...
#define NF(h)fract(sin(vec4(6,9,1,0)*h)*9e2)
vec4 RenderFirework1(vec2 uv)
{
	vec4 o;
	vec2 u=uv/iResolution.y;
	float e,d,i=-2.;
	for(vec4 p;i++<9.;d=floor(e=i*9.1+iTime),p=NF(d)+.3,e-=d)
	for(d=0.;d++<50.;)
	o+=p*(1.-e)/1e3/length(u-(p-e*(NF(d*i)-.5)).xy);
	u.y<NF(ceil(u.x*i+d+e)).x*.4?o:o;
	return o;
}

void mainImage(out vec4 o,in vec2 uv)
{
	vec4 cake=RenderCake(uv);
	o=cake;
	if(cake.a<.5)
	{
		vec4 firework=RenderFirework1(uv);
		o=max(o,firework);
	}
}