Shader "My/S_CloudRayMarching"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			//Cloud
			//--------------------------
			//_CloudAndSphere .W  雾的外轮廓
			float4 _CloudAndSphere;
			...
			float3 RayMarchingCloud(float3 ro, float3 rd, float t, float3 sceneCol, float depth, float4 cloudPos)
			{
				float4 sum = 0;
				float3 pos = ro + rd * t;
				...
				for (int i = 0; i < 30; ++ i)
				{
					...
					if (dist > cloudPos.w + 0.01 || sum.a > 0.99 || t > depth)
					{
						break;
					}
			...
			float4 RayMarching(Ray ray, float3 sceneCol, float depth)
			{
				...
				for (int i = 0; i < 64; ++ i)
				{
					...
					if (d < 0.01)//限制烟雾只能在球里面
					{
						...
						result = float4(RayMarchingCloud(p, ray.direction, d, sceneCol, depth, _CloudAndSphere), bestHit.position.w);
						break;
					}
					t += d;
				}
			...
			{
				float depth = LinearEyeDepth(tex2D(_CamerDepthTexture, i.uv).r);
